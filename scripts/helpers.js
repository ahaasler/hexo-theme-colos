...
hexo.extend.helper.register('list_categories', function(categories, options) {
	...
	var showCount = options.hasOwnProperty('show_count') ? options.show_count :
		true;
	...
	var childrenIndicator = options.hasOwnProperty('children_indicator') ?
		options.children_indicator : false;
	var result = '';
	var self = this;
	...
	function prepareQuery(parent) {
		...
		return categories.find(query).sort(orderby, order).filter(function(
			cat) {
			return cat.length;
		});
	}
	...
	function hierarchicalList(level, parent) {
		...
		prepareQuery(parent).forEach(function(cat, i) {
			...
			result += '<a class="' + className + '-list-link' + (isCurrent ?
					' current' : '') + '" href="' + self.url_for(cat.path) +
				'">';
			result += transform ? transform(cat.name) : cat.name;
			result += '</a>';

			if (showCount) {
				result += '<span class="' + className + '-list-count">' + cat
					.length + '</span>';
			}

			if (child) {
				result += '<ul class="' + className + '-list-child">' + child +
					'</ul>';
			}
	...
	function flatList(level, parent) {
		...
		prepareQuery(parent).forEach(function(cat, i) {
			...
			result += '<a class="' + className + '-link" href="' + self.url_for(
				cat.path) + '">';
			result += transform ? transform(cat.name) : cat.name;

			if (showCount) {
				result += '<span class="' + className + '-count">' + cat.length +
					'</span>';
			}
	...
	if (style === 'list') {
		result += '<ul class="' + className + '-list">' + hierarchicalList(0) +
			'</ul>';
	} else {
		result += flatList(0);
	}
